...
#define ALength 256
	...
	for (const CommandInfo* CI = g_commandInterface->Start(); CI < g_commandInterface->End(); CI++)
	{
		UInt32 i = CI->opcode;

		if (i && CI && CI->eval && (kNVSEOpcodeTest > i || (kNVSEOpcodeTest + 16 - 1) < i))
		{
			if (PI != g_commandInterface->GetParentPlugin(CI))	// Ok, this is bugged !!!
			{
				PI = g_commandInterface->GetParentPlugin(CI);
				if (PI)
					_MESSAGE("\n    // Added by %s", PI->name);
			};
