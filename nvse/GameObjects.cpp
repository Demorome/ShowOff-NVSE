...
{
	return (QuestObjectiveTargets *)ThisCall(s_PlayerCharacter_GetCurrentQuestTargets, this);
}
...
bool PlayerCharacter::SetSkeletonPath(const char* newPath)
{
	...
	ThisCall(s_TESObjectREFR_Set3D, this, NULL, true);
	...
	NiNode* newNode = (NiNode*)ThisCall(s_PlayerCharacter_GenerateNiNode, this, false);	// got a body WITHOUT the head :)
	...
	NiObject* cameraNode = (NiObject*)ThisCall(vfunc, newNode, "Camera3rd");				// returns NULL !!!
	*g_3rdPersonCameraNode = cameraNode;

	cameraNode = (NiObject*)ThisCall(vfunc, (NiNode*)this->playerNode, "Camera1st");		// returns NULL !!!
