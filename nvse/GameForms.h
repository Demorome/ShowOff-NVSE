...
public:
	...
	virtual void		SetRefID(uint32_t refID, bool generateID);
	...
	uint8_t GetOverridingModIdx() const;
...
public:
	...
	static uint32_t MaskForSlot(uint32_t slot);
	static uint32_t SlotForMask(uint32_t mask);
...
public:
	...
	virtual uint32_t	GetBaseActorValueInt(uint32_t avCode);		// GetBaseActorValue (used from Eval) result in EAX
	...
	virtual float	GetTempActorValue(uint32_t avCode);					// GetBaseActorValue04 (internal) result in st
	virtual float	GetActorValueDamage(uint32_t avCode);					// Returns the negative damage modifier
	virtual float	GetPermActorValue(uint32_t avCode);					// GetDamageActorValue or GetModifiedActorValue		called from Fn_08, result in st, added to Fn_01
	virtual uint32_t	GetNormalizedPermanentAV(uint32_t avCode);					// Manipulate GetPermanentActorValue, maybe convert to integer.
	...
	float GetThresholdedActorValue(uint32_t avCode) { return ThisCall<float>(0x66EF50, this, avCode); }
...
public:
	...
	virtual uint32_t	GetEyeHeight(void);
	virtual uint32_t	GetUnkShouldAttack(void);
...
public:
	...
	enum EAttackAnimations : uint8_t {
	...
	enum WeaponModEffects : uint32_t
	{
	...
	uint32_t GetItemModEffect(uint8_t which)	{ which -= 1; assert(which < 3); return effectMods[which]; }
	float GetItemModValue1(uint8_t which)		{ which -= 1; assert(which < 3); return value1Mod[which]; }
...
public:
	...
	virtual bool	Unk_4E(uint32_t arg1, uint32_t arg2, uint32_t arg3, uint32_t arg4);
...
public:
	...
	bool IsAddedObject(int32_t idx)
	{
...
public:
	...
	uint32_t					progress;		// 6C
	uint32_t					challengeflags;	// 70
	TESForm* SNAM;			// 74
	TESForm* XNAM;			// 78
};
// B0
...
public:
	...
	TESModel	model;				// 030
	uint32_t unk48[(0x60-0x48) >> 2]; // 048
};
...
class BGSCameraShot : public TESForm
{
	...
	uint32_t unk024[(0x78-0x24) >> 2];	// 024
};
...
public:
	...
	uint32_t			ambientRGB;			// 18
	uint32_t			directionalRGB;		// 1C
	uint32_t			fogRGB;				// 20
	float			fogNear;			// 24
	float			fogFar;				// 28
