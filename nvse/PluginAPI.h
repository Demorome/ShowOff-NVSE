...
public:
	...
	bool ExtractArgsV(void* null, ...)
	{
		va_list list;
		va_start(list, null);
		const auto result = g_expEvalUtils.ExtractArgsV(expEval, list);
		va_end(list);
		return result;
	}
	...
	void Error(const char* fmt, ...)
	{
		...
		g_expEvalUtils.ReportError(expEval, fmt, fmtArgs);
		va_end(fmtArgs);
	}
#endif
};
...
struct PluginScriptToken
{
	...
	bool CanConvertTo(Token_Type toType)
	{
		return g_expEvalUtils.ScriptTokenCanConvertTo(this, toType);
	}
