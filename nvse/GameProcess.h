...
public:
	...
	virtual void	Unk_F7();
	virtual void	Unk_F8();
	virtual int16_t	GetCurrentAnimAction();
	virtual int16_t	GetCurrentSequence();
	virtual void	Unk_FB();
	virtual void	Unk_FC();
	...
	virtual NiNode* GetLimbNode(uint32_t limbIdx); // taken from JIP LN
	...
	uint32_t			processLevel;	// 28	not initialized, only by descendant to 3 for Low, 2 for MiddleLow, 1 MiddleHighProcess and 0 for HighProcess
	Data2C* unk2C;			// 2C
};
...
struct AnimData
{
	...
	NiNode* nNeck1;			// 044
	float							unk048;				// 048
	uint16_t							animGroupIDs[8];		// 04C
	int32_t							sequenceState1[8];	// 05C
	int32_t							sequenceState2[8];	// 07C
	uint16_t							unk09C[8];			// 09C
	uint32_t							unk0AC[8];			// 0AC
	uint8_t							byte0CC;			// 0CC
	uint8_t							byte0CD;			// 0CD
	uint8_t							byte0CE;			// 0CE
	uint8_t							byte0CF;			// 0CF
